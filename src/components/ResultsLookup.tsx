    ...
    <section className="results-lookup">
      <div className="container">
        <div className="results-header">
          ...
          <p className="results-subtitle">Nhập mã bệnh nhân của bạn để xem kết quả xét nghiệm và khám bệnh</p>
        </div>
                  ...
                  {testResults.map((result) => (
                    <div key={result.id} className="result-card">
                      <div className="result-header">
                        <h4 className="result-test-name">{result.lab_tests.test_name}</h4>
                        <span className={`result-status status--${result.status}`}>
                          {result.status === 'completed' ? 'Hoàn thành' :
                           result.status === 'pending' ? 'Đang xử lý' : 'Đã xem xét'}
                        </span>
                      </div>
                      <p className="result-date">Ngày xét nghiệm: {formatDate(result.test_date)}</p>
                      ...
                      {result.reviewed_by && (
                        <p className="result-reviewer">Người xem xét: {result.reviewed_by}</p>
                      )}
                    </div>
                  ))}
                </div>
                  ...
                  {medicalExams.map((exam) => (
                    <div key={exam.id} className="exam-card">
                      <div className="exam-header">
                        <h4 className="exam-date">Khám ngày: {formatDate(exam.exam_date)}</h4>
                        <span className="exam-doctor">BS: {exam.doctor_name}</span>
                      </div>
